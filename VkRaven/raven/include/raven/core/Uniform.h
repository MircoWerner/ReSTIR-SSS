...
namespace raven {
    ...
    public:
        ...
        static std::shared_ptr<Uniform> reflect(GPUContext *gpuContext, const SpvReflectDescriptorBinding &binding) {
            assert(static_cast<vk::DescriptorType>(binding.descriptor_type) == vk::DescriptorType::eUniformBuffer);
        ...
        void create() {
            ...
            for (size_t i = 0; i < m_gpuContext->getMultiBufferedCount(); i++) {
                ...
                auto settings = Buffer::BufferSettings{.m_sizeBytes = getBytes(), .m_bufferUsages = vk::BufferUsageFlagBits::eUniformBuffer, .m_memoryProperties = vk::MemoryPropertyFlagBits::eHostVisible | vk::MemoryPropertyFlagBits::eHostCoherent, .m_name=ss.str()};
        ...
        void generateBufferInfo(vk::DescriptorBufferInfo *bufferInfo, uint32_t index) {
