...
namespace raven {
    class PassGBuffer final : public PassShaderCompute {
    public:
        explicit PassGBuffer(GPUContext *gpuContext) : PassShaderCompute(gpuContext, "PassGBuffer") {
        }
    ...
    protected:
        std::vector<std::shared_ptr<Shader>> createShaders() override {
            ...
            return {std::make_shared<Shader>(m_gpuContext, Paths::m_resourceDirectoryPath + "/shaders/main/passes", "pass_gbuffer.comp", vk::ShaderStageFlagBits::eCompute, workGroupSize, specializationMapEntries, specializationData)};
        }
