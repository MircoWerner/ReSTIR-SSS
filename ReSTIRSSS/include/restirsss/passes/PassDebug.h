...
namespace raven {
    class PassDebug final : public PassShaderCompute {
    public:
        explicit PassDebug(GPUContext *gpuContext) : PassShaderCompute(gpuContext, "PassDebug") {
        }
    ...
    protected:
        std::vector<std::shared_ptr<Shader>> createShaders() override {
            ...
            return {std::make_shared<Shader>(m_gpuContext, Paths::m_resourceDirectoryPath + "/shaders/main/passes", "pass_debug.comp", vk::ShaderStageFlagBits::eCompute, workGroupSize, specializationMapEntries, specializationData)};
        }
