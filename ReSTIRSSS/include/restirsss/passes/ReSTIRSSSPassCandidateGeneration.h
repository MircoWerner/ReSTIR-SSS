...
namespace raven {
    class ReSTIRSSSPassCandidateGeneration final : public PassShaderCompute {
    public:
        explicit ReSTIRSSSPassCandidateGeneration(GPUContext *gpuContext) : PassShaderCompute(gpuContext, "ReSTIRSSSPassCandidateGeneration") {
        }
    ...
    protected:
        std::vector<std::shared_ptr<Shader>> createShaders() override {
            ...
            return {std::make_shared<Shader>(m_gpuContext, Paths::m_resourceDirectoryPath + "/shaders/main/passes", "restirsss_pass_candidate_generation.comp", vk::ShaderStageFlagBits::eCompute, workGroupSize, specializationMapEntries, specializationData)};
        }
