...
namespace raven {
    class ReSTIRSSSPassShade final : public PassShaderCompute {
    public:
        explicit ReSTIRSSSPassShade(GPUContext *gpuContext) : PassShaderCompute(gpuContext, "ReSTIRSSSPassShade") {
        }
    ...
    protected:
        std::vector<std::shared_ptr<Shader>> createShaders() override {
            ...
            return {std::make_shared<Shader>(m_gpuContext, Paths::m_resourceDirectoryPath + "/shaders/main/passes", "restirsss_pass_shade.comp", vk::ShaderStageFlagBits::eCompute, workGroupSize, specializationMapEntries, specializationData)};
        }
